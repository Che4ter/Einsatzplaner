...
export function renderEventCard(ev: any, team: any[], month: number): string {
  ...
  const teamById = Object.fromEntries(team.map((t: any) => [t.id, t]));

  const pips = Array.from({length: need}, (_: unknown, i: number) =>
    `<span class="meter-pip${i < assigned.length ? ' filled ' + tone : ''}"></span>`
  ).join('');

  const assignedChips = assigned.map((id: string) => {
    const m = teamById[id];
    if (!m) return '';
...
export function renderClosedCard(ev: any, month: number): string {
...
const ACTION_ICON_NAME: Record<string, string> = {
...
export function renderActivityEntry(e: any, teamById: Record<string, any>, today: string): string {
  ...
  const humanFieldValue = (field: string, val: any): string => {
    if (!val && val !== 0) return '—';
  ...
  const personChip = (id: string): string => {
...
export function fmtDayHeading(dayIso: string, todayIso: string): { kicker: string | null; body: string; isToday: boolean } {
...
export function renderQAPopover(team: any[], assignedStaff: string[], eventId: string, month: number): string {
  const rows = team.filter((m: any) => m.active).slice().sort((a: any, b: any) => a.name.localeCompare(b.name)).map((m: any) => {
